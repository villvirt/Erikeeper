...
public class GetLeaderBoards : MonoBehaviour {
	...
	private IEnumerator getLeaderBoardsCorout(){
		...
		FBContainer.scoreBoardService.GetTopNRankers(gameName,max, new LeaderboardsCallBack());
	...
	public void getNext(){
		if(currentIndex<10 && ActiveScoreList.Count>((currentIndex+1)*10)){
			setText(currentIndex+1);
			currentIndex=currentIndex+1;
			Debug.Log("Current index"+currentIndex);
			Up.interactable = true;
			if (currentIndex == 9 || ActiveScoreList.Count - (currentIndex * 10) < 10) {
				Down.interactable = false;
			}
		}

		else{
			Debug.Log ("activelist: "+ActiveScoreList.Count.ToString());
	...
	private void setText(int index){
		...
		Scores.GetComponent<UnityEngine.UI.Text>().text=scores;
			...
			Ranks.GetComponent<UnityEngine.UI.Text>().text="";
			Names.GetComponent<UnityEngine.UI.Text>().text="Ei tuloksia";
			Scores.GetComponent<UnityEngine.UI.Text>().text="";
		}
	}
